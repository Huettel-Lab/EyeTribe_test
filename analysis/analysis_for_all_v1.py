...
OPTIONS = ['validation'] #, 'images', 'saccadometry', 'samplerate', 'pupilometry']

# excluded participants
EXCL = [] #['ED_pupil']

# settings
TRACKERS = ['tobii'] #'eyelink', 'eyetribe', 'tobii']
...
if 'validation' in OPTIONS:
	...
	# ALL SAMPLES PLOT horizontal
	print("Plotting all horizontal validation samples.")
	# open new figure
	...
	for ppname in PPS:
		...
		for trackertype in TRACKERS:
			# load their data
			x = numpy.load(os.path.join(IOUTDIR, ppname, trackertype, 'validation_horizontal_x.npy'))
			y = numpy.load(os.path.join(IOUTDIR, ppname, trackertype, 'validation_horizontal_y.npy'))
			# plot all samples
	...
	for i in range(len(CALIBPOINTShor)):
		...
		ax.plot(CALIBPOINTShor[i][0], CALIBPOINTShor[i][1], 'o', markersize=6, c=PLOTCOLS['validation'], alpha=0.5)
	# finish figure
	...
	# save plot
	fig.savefig(os.path.join(OUTDIR, "validation_all_horizontal_samples.png"))
	# close figure
	pyplot.close(fig)
	
	
	# ALL SAMPLES PLOT vertical
	print("Plotting all vertical validation samples.")
	# open new figure
	fig, ax = pyplot.subplots(nrows=1, ncols=1)
	fig.set_dpi(DPI)
	fig.set_size_inches(FIGSIZE, forward=True)
	...
	for ppname in PPS:
		...
		for trackertype in TRACKERS:
			# load their data
			x = numpy.load(os.path.join(IOUTDIR, ppname, trackertype, 'validation_vertical_x.npy'))
			y = numpy.load(os.path.join(IOUTDIR, ppname, trackertype, 'validation_vertical_y.npy'))
			# plot all samples
			ax.plot(x, y, 'o', markersize=1, markeredgewidth=0, c=PLOTCOLS[trackertype])
	# loop through calibration points
	for i in range(len(CALIBPOINTSver)):
		# plot validation points
		ax.plot(CALIBPOINTSver[i][0], CALIBPOINTSver[i][1], 'o', markersize=6, c=PLOTCOLS['validation'], alpha=0.5)
	# finish figure
	ax.axis([0, DISPSIZE[0], 0, DISPSIZE[1]])
	ax.invert_yaxis()
	#oax.legend()
	...
	ax.set_ylabel("display y coordinate (pixels)")
	# save plot
	fig.savefig(os.path.join(OUTDIR, "validation_all_vertical_samples.png"))
	# close figure
	pyplot.close(fig)
