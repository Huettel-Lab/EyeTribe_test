...
read_data = {'eyelink':read_edf, 'eyetribe':read_eyetribe, 'tobii':read_tobii}
...
for trackertype in ['tobii']: #TODO adjust for all eyetrackers
	...
	if DOVALIDATION:
		...
		# READ DATA
		validationdata = read_data[trackertype](DATAFILE, "validation_horizontal_point_fix", stop="validation_horizontal_point_off", missing=0.0, debug=False)
		...
		for i in range(len(validationdata)):
			...
			sysl['x'].extend(list(validationdata[i]['x'][validationdata[i]['x']>0] - CALIBPOINTShor[i][0]))
			sysl['y'].extend(list(validationdata[i]['y'][validationdata[i]['y']>0] - CALIBPOINTShor[i][1]))
			...
			ax.plot(CALIBPOINTShor[i][0], CALIBPOINTShor[i][1], 'o', markersize=12, c=PLOTCOLS['validation'], label='stimulus')
			oax.plot(CALIBPOINTShor[i][0], CALIBPOINTShor[i][1], 'o', markersize=12, c=PLOTCOLS['validation'])
			soax.plot(CALIBPOINTShor[i][0], CALIBPOINTShor[i][1], 'o', markersize=6, c=PLOTCOLS['validation'], alpha=0.5)
			# finish plot
			ax.axis([0, DISPSIZE[0], 0, DISPSIZE[1]])
			ax.invert_yaxis()
			ax.legend()
			# save plot
			fig.savefig(os.path.join(outputdir, "validationpoint_horizontal_%d_%d.png" % (CALIBPOINTShor[i][0], CALIBPOINTShor[i][1])))
			# close figure
			pyplot.close(fig)

		# save files with x and y coordinates
		numpy.save(os.path.join(outputdir, 'validation_horizontal_x.npy'), allx)
		numpy.save(os.path.join(outputdir, 'validation_horizontal_y.npy'), ally)
		...
		#oax.legend()
		# save plot
		ofig.savefig(os.path.join(outputdir, "validationpoint_horizontal_all.png"))
		# close figure
		pyplot.close(ofig)
		...
		# READ DATA
		validationdata = read_data[trackertype](DATAFILE, "validation_vertical_point_fix", stop="validation_vertical_point_off", missing=0.0, debug=False)
		...
		for i in range(len(validationdata)):
			...
			randl['x'].extend(list(numpy.diff(validationdata[i]['x'][validationdata[i]['x']>0])))
			randl['y'].extend(list(numpy.diff(validationdata[i]['y'][validationdata[i]['y']>0])))
			sysl['x'].extend(list(validationdata[i]['x'][validationdata[i]['x']>0] - CALIBPOINTSver[i][0]))
			sysl['y'].extend(list(validationdata[i]['y'][validationdata[i]['y']>0] - CALIBPOINTSver[i][1]))
			...
			soax.plot(validationdata[i]['x'], validationdata[i]['y'], 'o', markersize=1, markeredgewidth=0, c=PLOTCOLS[trackertype])
			# plot validation point
			ax.plot(CALIBPOINTSver[i][0], CALIBPOINTSver[i][1], 'o', markersize=12, c=PLOTCOLS['validation'], label='stimulus')
			oax.plot(CALIBPOINTSver[i][0], CALIBPOINTSver[i][1], 'o', markersize=12, c=PLOTCOLS['validation'])
			soax.plot(CALIBPOINTSver[i][0], CALIBPOINTSver[i][1], 'o', markersize=6, c=PLOTCOLS['validation'], alpha=0.5)
			# finish plot
			ax.axis([0, DISPSIZE[0], 0, DISPSIZE[1]])
			ax.invert_yaxis()
			ax.legend()
			# save plot
			fig.savefig(os.path.join(outputdir, "validationpoint_vertical_%d_%d.png" % (CALIBPOINTSver[i][0], CALIBPOINTSver[i][1])))
			# close figure
			pyplot.close(fig)

		# save files with x and y coordinates
		numpy.save(os.path.join(outputdir, 'validation_vertical_x.npy'), allx)
		numpy.save(os.path.join(outputdir, 'validation_vertical_y.npy'), ally)
		...
		#oax.legend()
		# save plot
		ofig.savefig(os.path.join(outputdir, "validationpoint_vertical_all.png"))
		# close figure
		pyplot.close(ofig)
		...
		for d in ['x', 'y']:
			...
			RMSall[d] = cm2ang(((numpy.array(randl[d])**2)**0.5)/PIXPERCM, SCREENDIST)
			accall[d] = cm2ang(numpy.array(sysl[d])/PIXPERCM, SCREENDIST)
		# write a report to a new text file
		
		txtf.write(OUTSEP.join(map(str, [RMS['x'], RMS['y'], acc['x'], acc['y'], 'mean'])) + '\n')

		RMSxAvg = 0.0
		RMSyAvg = 0.0
		ACCxAvg = 0.0
		ACCyAvg = 0.0
		for i in range(0, min(len(RMSall['x']), len(RMSall['y']), len(accall['y']), len(accall['x']))-1):
			RMSxAvg += RMSall['x'][i]
			RMSyAvg += RMSall['y'][i]
			ACCxAvg += accall['x'][i]
			ACCyAvg += accall['y'][i]
			...
			if i % 120 == 0:
				txtf.write(OUTSEP.join(map(str, [RMSxAvg/120.0, RMSyAvg/120.0, ACCxAvg/120.0, ACCyAvg/120.0, 'point #', i/120])) + '\n')
				RMSxAvg = 0.0
				RMSyAvg = 0.0
				ACCxAvg = 0.0
				ACCyAvg = 0.0
	...
	if DOPUPILOMETRY:
		...
		# PARSE DATA
		for i in range(len(pupildata)-1):
			# conditions
