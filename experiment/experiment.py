...
for i in range(len(CALIBPOINTShor)):
    ...
    x, y = CALIBPOINTShor[i]
        ...
        clock.pause(1000)
        tracker.log("validation_horizontal_point_fix")
        # wait for a bit
        ...
        #disp.show()
        # stop recording
        tracker.log("validation_horizontal_point_off")
...
scr.clear()
scr.draw_line((0,0,0), (int(0.15*DISPSIZE[0]),int(0.05*DISPSIZE[1])), (int(0.15*DISPSIZE[0]),int(0.95*DISPSIZE[1])), 1)
scr.draw_line((0,0,0), (int(0.15*DISPSIZE[0]),int(0.95*DISPSIZE[1])), (int(0.5*DISPSIZE[0]),int(0.95*DISPSIZE[1])), 1)
scr.draw_line((0,0,0), (int(0.5*DISPSIZE[0]),int(0.95*DISPSIZE[1])), (int(0.5*DISPSIZE[0]),int(0.05*DISPSIZE[1])), 1)
scr.draw_line((0,0,0), (int(0.5*DISPSIZE[0]),int(0.05*DISPSIZE[1])), (int(0.85*DISPSIZE[0]),int(0.05*DISPSIZE[1])), 1)
scr.draw_line((0,0,0), (int(0.85*DISPSIZE[0]),int(0.05*DISPSIZE[1])), (int(0.85*DISPSIZE[0]),int(0.95*DISPSIZE[1])), 1)

# loop through points
for i in range(len(CALIBPOINTSver)):
    # get coordinate
    x, y = CALIBPOINTSver[i]
        # draw calibration point
        scr.draw_fixation(fixtype='cross', pos=(x,y))
...
for i in range(len(CALIBPOINTSver)):
    ...
    x, y = CALIBPOINTSver[i]
        ...
        # allow for a bit of time so the subject can fixate the target
        clock.pause(1000)
        tracker.log("validation_vertical_point_fix")
        # wait for a bit
        clock.pause(POINTTIME)
        # clear screen
        #scr.clear()
        ...
        #disp.show()
        # stop recording
        tracker.log("validation_vertical_point_off")
        tracker.stop_recording()
        # inter-trial interval
        clock.pause(ITIval)
